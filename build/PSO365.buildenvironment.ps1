...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        Encoding = 'utf8'
        ModuleToBuild = 'PSO365'
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://github.com/rdtechie/PSO365'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Richard Diphoorn. All rights reserved."
        ModuleLicenseURI = 'https://github.com/rdtechie/PSO365/LICENSE.md'
        ModuleTags = 'Office 365,Management,Provisioning' -split ','
        ModuleAuthor = 'Richard Diphoorn'
        ModuleDescription = 'Office 365 Management'
        ...
        # You should NOT enter this key here but rather manually enter it in the PSO365.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "PSO365.buildenvironment.json"
